                 ...
                 T& rng)
{
  ...
  std::vector<Critter> t_parents(begin(parents), begin(parents) + parent_count);
  ...
  for (auto i : itertools::range(population_count)) {
    ...
    k_point_crossover(t_parents, t_children, probability_distb, ixs, N, rng);
  }
...
main(int argc, char** argv)
{
  ...
  auto rng_state =
    _rng_state > 0 ? static_cast<uint64_t>(_rng_state) : epoch_time_us.count();
