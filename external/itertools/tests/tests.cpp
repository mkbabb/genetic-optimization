...
main()
{
    ...
    //   time_multiple_tests();
    //   to_string_tests();
    std::vector<std::tuple<int, int>> t = {{1, 2}, {3, 4}, {5, 6}, {7, 8}};

    auto r = itertools::range(t.size());

    for (auto v : itertools::zip(r, t)) {
        auto [n, i, j] = tupletools::flatten(v);
        fmt::print("{} {} {}\n", n, i, j);
    }
    for (auto i : itertools::range(10)) { std::cout << i << std::endl; }
